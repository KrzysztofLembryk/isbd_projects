...
fn test_create_new_empty() 
{
    ...
    assert_eq!(ok_col.col_type(), AllowedColTypes::IntType);
    ...
    assert!(ColHeader::new_empty(AllowedColTypes::IntType, not_ascii).is_err());
    assert!(ColHeader::new_empty(AllowedColTypes::IntType, too_long).is_err());
}
...
fn test_create_next()
{
    ...
    assert_eq!(next_col.col_type(), AllowedColTypes::IntType);
...
fn test_save_to_file_wrong_paths()
{
    ...
    let col_h = ColHeader::new_empty(AllowedColTypes::IntType, col_name).unwrap();
